...
import { ZemanFunctions, lDTFromFunc } from '../ROYZmanimBinding.js';
import n2wordsOrdinal from '../misc/n2wordsOrdinal.js';
import { newGeolocation, uniffiInitAsync } from '../../libraries/zmanim-binding/index.js';
...
async function messageHandler (x) {
	...
	try {
		const geoLocation = newGeolocation(x.data.geoCoordinates[1], x.data.geoCoordinates[2], x.data.geoCoordinates[3]);
		const timeZone = x.data.geoCoordinates[4];
		...
		/** @type {[Intl.DateTimeFormatOptions, options?: Intl.DateTimeFormatOptions]} */
		const defaulTF = [{
		...
		zmanCalc.setDate(Luxon.DateTime.fromISO(x.data.date, { zone: timeZone }));
		...
		const yomTovObj = {
			...
			// Semi-Holidays & Fasts
			[WebsiteLimudCalendar.PESACH_SHENI]: {
				hb: "פסח שני",
				en: "Pesaḥ Sheni",
				"en-et": "Pesaḥ Sheni"
			},
			[WebsiteLimudCalendar.LAG_BAOMER]: {
				hb: "לג בעומר",
				en: "Lag Baomer",
				"en-et": "Lag Baomer"
			},
			[WebsiteLimudCalendar.TU_BEAV]: {
				"he": 'ט"ו באב',
				en: "Tu Be'av",
				"en-et": "Tu Be'av"
			},
			[WebsiteLimudCalendar.TU_BESHVAT]: {
				"he": 'ט"ו בשבת',
				en: "Tu Bishvath",
				"en-et": "Tu Bishvath"
			},
			[WebsiteLimudCalendar.PURIM_KATAN]: {
				hb: "פורים קתן",
				en: "Purim Katan",
				"en-et": "Purim Katan"
			},
			[WebsiteLimudCalendar.SHUSHAN_PURIM_KATAN]: {
				hb: "שושן פורים קתן",
				en: "Shushan Purim Katan",
				"en-et": "Shushan Purim Katan"
			},
			[WebsiteLimudCalendar.PURIM]: {
				hb: "פורים",
				en: "Purim",
				"en-et": "Purim"
			},
			[WebsiteLimudCalendar.SHUSHAN_PURIM]: {
				hb: "שושן פורים",
				en: "Shushan Purim",
				"en-et": "Shushan Purim"
			},
			...
			[WebsiteLimudCalendar.YOM_HASHOAH]: {
				hb: "יום השועה",
				"en-et": "Yom Hashoa",
				en: "Holocaust Memorial Day"
			},
			[WebsiteLimudCalendar.YOM_HAZIKARON]: {
				hb: "יום הזכרון",
				"en-et": "Yom Hazikaron",
				en: "Day of Rememberance"
			},
			[WebsiteLimudCalendar.YOM_HAATZMAUT]: {
				hb: "יום האצמעות",
				"en-et": "Yom Haatzmauth",
				en: "Yom Haatzmauth"
			}, // Tachanun is said
			[WebsiteLimudCalendar.YOM_YERUSHALAYIM]: {
				hb: "יום ירושלים",
		...
		function handleShita (/** @type {string} */ shita) {
			...
			/**
			 * @param {import('luxon').DateTime} zDT
			...
			switch (shita) {
				case 'getMolad':
					...
					if (jCal.getDate().dayOfYear == jCal.tomorrow().tomorrow().getMoladAsDate().withTimeZone(timeZone).dayOfYear)
						renderZmanInDiv(
							zDT2lDT(jCal.tomorrow().tomorrow().getMoladAsDate().withTimeZone(timeZone)),
							{
					...
					if (jCal.getDate().dayOfYear == jCal.getTchilasZmanKidushLevana3Days().withTimeZone(timeZone).dayOfYear) {
						let time = zDT2lDT(jCal.getTchilasZmanKidushLevana3Days().withTimeZone(timeZone));
						const bLzmanCalc = zmanCalc.chainDate(time);
					...
					if (jCal.getDate().dayOfYear == jCal.getTchilasZmanKidushLevana7Days().withTimeZone(timeZone).dayOfYear) {
						let time = zDT2lDT(jCal.getTchilasZmanKidushLevana7Days().withTimeZone(timeZone));
						const bLzmanCalc = zmanCalc.chainDate(time);
					...
					let sameTime = false;
					if (jCal.getDate().dayOfYear == jCal.getSofZmanKidushLevanaBetweenMoldos().withTimeZone(timeZone).dayOfYear) {
						let time = zDT2lDT(jCal.getSofZmanKidushLevanaBetweenMoldos().withTimeZone(timeZone));
						const bLzmanCalc = zmanCalc.chainDate(time);

						if (rangeTimes(bLzmanCalc.getAlotHashahar(), time, bLzmanCalc.getTzet()))
							time = bLzmanCalc.getAlotHashahar();

						if (time.reconfigure({ outputCalendar: "hebrew" }).day == 15 && time.equals(bLzmanCalc.getAlotHashahar()))
							sameTime = true;
				...
				case 'special':
					...
					if (jCal.getDayOfWeek() == 1
					&& jCal.getJewishMonth() == (jCal.isJewishLeapYear() ? 13 : 12)
				...
				case 'candleLightingRT':
					if (jCal.hasCandleLighting()) {
						...
						if (jCal.getDayOfWeek() === 6 || !jCal.isAssurBemelacha())
							...
							renderZmanInDiv(lDTFromFunc(zmanCalc.getTzetMelakha()), candleConfig);
					...
					if (x.data.allShitot.includes('getTzetHumra') && jCal.isTaanis() && jCal.getJewishMonth() == WebsiteLimudCalendar.AV && jCal.getDayOfWeek() == 1)
						return false;

					if (!jCal.hasCandleLighting() && jCal.isAssurBemelacha()) {
						renderZmanInDiv(lDTFromFunc(zmanCalc.getTzetMelakha()), {dtF: defaulTF, icon: icons.havdalah, hideAMPM: true});
				...
				case 'getTzet':
					if (jCal.hasCandleLighting() || !jCal.isAssurBemelacha()) {
						...
						if (jCal.tomorrow().getDayOfChanukah() !== -1 && jCal.getDayOfWeek() !== 6) {
							renderZmanInDiv(zmanCalc.getTzet().plus({ minutes: 30 }), {dtF: defaulTF, icon: "🕎" + {
								'hb': " לפני",
								"en-et": " before",
								'en': " before"
							}[x.data.lang], hideAMPM: true})
				...
				case 'getTzetHumra':
					...
					const havdalahOnWine = jCal.isTaanis() && jCal.getJewishMonth() == WebsiteLimudCalendar.AV && jCal.getDayOfWeek() == 1;
				...
				case 'getNetz':
					...
					if (!(sunriseTime instanceof Luxon.DateTime)) {
				...
				case 'getHatzoth':
					...
					if (jCal.isYomKippur()) {
						renderZmanInDiv(zmanCalc.timeRange.current.sunrise.plus(zmanCalc.fixedHourToSeasonalHour(7)), {dtF: defaulTF, icon: '(' + {
							'hb': "תסיים מוסף לפני ",
		...
		for (let index = 1; index <= halfDaysInMonth; index++) {
			...
			zmanCalc.setDate(zDT2lDT(jCal.getDate().toZonedDateTime(timeZone)))

			if (jCal.getYomTovIndex() == WebsiteLimudCalendar.EREV_PESACH)
				hamesDate = zmanCalc.date;
		...
		/*
		if (!initTekuf.equals(zmanCalc.nextTekufa(zmanCalc.config.fixedMil))
		&& thisMonthFooter.lastElementChild.hasAttribute('data-zyfooter-tekufa')) {
			...
			|| (thisMonthFooter.lastElementChild.getAttribute('data-zyfooter-tekufa') == 'beforeHatzot'
				...
				const nextTekufaJDate = [1, 4, 7, 10]
					...
					.sort((jDateA, jDateB) => {
						const durationA = initTekuf.until(jDateA.getDate().toZonedDateTime("+02:00").withTimeZone(timeZone))
						const durationB = initTekuf.until(jDateB.getDate().toZonedDateTime("+02:00").withTimeZone(timeZone))

						return Math.abs(durationA.toDuration().days) - Math.abs(durationB.toDuration().days)
					})[0]
				...
				tekufaTimingDiv.appendChild(document.createTextNode({
					...
					initTekuf.minus({ minutes: 30 }).toLocaleString(...defaulTF),
					initTekuf.plus({ minutes: 30 }).toLocaleString(...defaulTF),
				].join('-')));

				if (nextTekufaJDate.getJewishMonth() == WebsiteLimudCalendar.TISHREI && !x.data.israel) {
					tekufaTimingDiv.appendChild(document.createElement("br"));
					tekufaTimingDiv.appendChild(document.createTextNode({
						"en": "Switch to ברך עלינו on ",
						"hb": "תחליף לברך עלינו ביום ",
						"en-et": "Switch to ברך עלינו on "
					}[x.data.lang] + formatDate(initTekuf.plus({ days: 60 }))))
				}
		...
		if (thisMonthFooter.lastElementChild.hasAttribute('data-zyfooter-levana')) {
			...
			for (const jMonthForBLevana of jMonthsForBLevana) {
				...
				bLTimes.appendChild(document.createTextNode({
					...
					"en": "Beginning: ",
					"en-et": "Beginning: "
				}[x.data.lang] + jCalBMoon.getTchilasZmanKidushLevana7Days().withTimeZone(timeZone).toLocaleString(...dtFBLevana)));
				bLTimes.appendChild(document.createElement("br"));
				bLTimes.appendChild(document.createTextNode({
					"hb": 'סוף (רמ"א): ',
					"en": 'End (Rama): ',
					"en-et": "End (Rama): "
				}[x.data.lang] + jCalBMoon.getSofZmanKidushLevanaBetweenMoldos().withTimeZone(timeZone).toLocaleString(...dtFBLevana)));
 ...
 * @param {import('luxon').DateTime} middle
 * @param {import('luxon').DateTime} end
 * @param {boolean} inclusive
 * @returns {boolean}
 */
function rangeTimes(start, middle, end, inclusive = true) {
    const acceptedValues = [1]; // 1 means "greater than"
    if (inclusive) acceptedValues.push(0); // 0 means "equal to"

    const compare = (/** @type {number} */ a, /** @type {number} */ b) => a < b ? -1 : a > b ? 1 : 0;

    return acceptedValues.includes(compare(middle.toMillis(), start.toMillis())) &&
           acceptedValues.includes(compare(end.toMillis(), middle.toMillis()));
}

/**
 * @param {import('temporal-polyfill').Temporal.ZonedDateTime} zdt
 */
function zDT2lDT(zdt) {
	return Luxon.DateTime.fromMillis(zdt.epochMilliseconds).setZone(zdt.timeZoneId)
}

/** @param {import('luxon').DateTime} date */
function formatDate(date) {
	return `${daysForLocale('en')[date.weekday]}, ${monthForLocale('en')[date.month]} ${getOrdinal(date.day, true)}`
}