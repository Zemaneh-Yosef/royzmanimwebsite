...
/**
 * @typedef {Object} TzeithIssurMelakha
 * @property {number} minutes - Offset duration in minutes.
 * @property {number} degree - Solar depression angle in degrees.
 */

/**
 * @typedef {Object} CustomTimes
 * @property {number} candleLighting - Minutes before sunset for candle lighting.
 * @property {TzeithIssurMelakha} tzeithIssurMelakha - Parameters for nightfall/end of forbidden work.
 */

/**
 * @typedef {Object} CalendarToggle
 * @property {boolean} rtKulah - Toggles Rabbeinu Tam leniency calculation.
 * @property {string} tekufaMidpoint - Midpoint reference method (e.g., "hatzoth").
 * @property {string} tekufaCalc - Calculation method used for season changes (e.g., "shemuel").
 * @property {boolean} forceSunSeasonal - Forces seasonal calculations based directly on the sun.
 */

/**
 * @typedef {Object} WeeklyPrint
 * @property {number} year - Year.
 * @property {number|"current"} month - month.
 * @property {boolean} noTzetStrict
 * @property {string} nationality - Community tradition or rite (e.g., "Syrian").
 * @property {number} addExtraHiloulot - Number of additional commemorative anniversaries to include.
 */

/**
 * @typedef {Object} VSunrise
 * @property {number[]} times - Array of UNIX timestamps for visible sunrise calculations.
 * @property {string} url - Source URL for ChaiTables astronomical calculations.
 */

/**
 * @typedef {Object} LocationConfig
 * @property {string} name - Name of the target location.
 * @property {number} lat - Latitude coordinate.
 * @property {number} long - Longitude coordinate.
 * @property {number} elevation - Elevation in meters above sea level.
 * @property {string} timezone - IANA timezone identifier.
 * @property {VSunrise} vsunrise - Visible sunrise metadata and source data.
 */

/**
 * @typedef {Object} ZmanimConfig
 * @property {boolean} seconds - Whether to render seconds in time displays.
 * @property {("h12"|"h24")} timeFormat - Hour format standard.
 * @property {"hb"|"en"} language - Two-letter ISO language code.
 * @property {CalendarToggle} calendarToggle - Rabbinic calculation toggles.
...
const printConfigParam = new URLSearchParams(window.location.search);
/** @type {ZmanimConfig} */
const settings = (await (await fetch(`/assets/weeklyConfig/${printConfigParam.get('config')}.json`)).json());

/** @type {'iso8601'|'hebrew'} */
let cal;
if ("year" in settings.weeklyPrint) {
	cal = settings.weeklyPrint.year > 4000 ? "hebrew" : "iso8601";
} else {
	cal = settings.language == "hb" ? "hebrew" : "iso8601"
}

/** @type {{month: number; year: number}} */
const dateForCal = {
	month: undefined,
	year: ("year" in settings.weeklyPrint ? settings.weeklyPrint.year : Temporal.Now.plainDateISO().withCalendar(cal).year),
};

if ("month" in settings.weeklyPrint) {
	if (typeof settings.weeklyPrint.month == "number")
		dateForCal.month = settings.weeklyPrint.month;
	else if (!("year" in settings.weeklyPrint))
		dateForCal.month = Temporal.Now.plainDateISO().withCalendar(cal).month
	else
		throw new Error("No mix specified-year and currentMonth");
} else {
	dateForCal.month = 1
}

const useOhrHachaim = settings.calendarToggle.forceSunSeasonal

/** @type {HTMLElement} */
const baseTable = document.getElementById('templateZmanim');
baseTable.removeAttribute("id")

if (useOhrHachaim || settings.weeklyPrint.noTzetStrict) {
...
const endDate = Temporal.Now.plainDateISO()
	.withCalendar(cal)
	.with({ month: 1, day: 1, year: dateForCal.year + ("continueToNext" in settings.weeklyPrint ? 2 : 1) })
	.subtract({ days: 1 })
...
if ("continueToNext" in settings.weeklyPrint) {
